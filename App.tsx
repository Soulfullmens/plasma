      ...
      <header className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-mono -mt-1 uppercase tracking-tighter">Scientific Feasibility Artifact</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 text-[10px] text-indigo-400 font-mono font-bold">
             <span className="inline-block w-2 h-2 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.8)]"></span>
             PHASE_1_LOCKED
          </div>
          <div className="h-8 w-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center overflow-hidden grayscale hover:grayscale-0 transition-all cursor-help" title="Phase 1 Verified">
          ...
          {activeTab === 'simulation' && (
            <div className="h-full flex flex-col">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {[
                  { label: "Status", value: "TRL-3", sub: "Reproducible POC", color: "indigo" },
                  { label: "Error Residue", value: "O(10⁻³)", sub: "Within Analytic Range", color: "emerald" },
                  { label: "Verification", value: "FROZEN", sub: "No Scope Creep", color: "amber" }
                ].map((stat, i) => (
          ...
          {activeTab === 'roadmap' && (
            <div className="h-full bg-slate-900/50 border border-slate-800 rounded-xl p-8 overflow-y-auto">
              <div className="max-w-3xl">
                <h2 className="text-2xl font-bold mb-4">Phase 1 Frozen Methodology</h2>
                <p className="text-slate-400 text-sm mb-8 leading-relaxed italic">
                  This methodology is locked for publication. Any modification to dimensions or EM coupling must occur in the Phase 2 branch. The artifacts provided here reproduce the falsification of zero-closure truncation in 1D-1V electrostatic systems.
                </p>
                    ...
                    <div key={phase}>
                      ...
                      <div className="grid grid-cols-1 gap-3">
                        {ROADMAP_DATA.filter(d => d.phase === phase).map(step => (
                          <div key={step.id} className="flex gap-4 p-4 rounded-xl bg-slate-900/40 border border-slate-800/40">
                            <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0 bg-emerald-500/10 text-emerald-500">
                              <ICONS.Check />
                            </div>
                            <div>
                              <div className="flex items-center gap-3 mb-1">
                                <h4 className="font-semibold text-slate-300 text-xs tracking-tight">{step.title}</h4>
                                <span className="text-[8px] px-1.5 py-0.5 rounded-full uppercase font-mono bg-slate-950 text-emerald-500 border border-emerald-900/50">
                                  Verified
                                </span>
                              </div>
